    ...
    @foreach($sellerOrder as $vendor)
        ...
        <table class="table-bordered">
            ...
            @foreach($vendor['items'] as $item)
                @php
                    $addons = $item->orderItem?->addons ?? collect();
                @endphp
                <tr>
                    ...
                    <td>
                        ...
                        @if($addons->isNotEmpty())
                            <div style="margin-top: 6px;">
                                <b>{{ __('labels.addons') }}:</b>
                                <ul style="margin: 2px 0 0 16px; padding: 0;">
                                    @foreach($addons as $addon)
                                        <li>
                                            @if(!empty($addon->addonGroup?->title))
                                                {{ $addon->addonGroup->title }} —
                                            @endif
                                            {{ $addon->addonItem?->title ?? '—' }}
                                            · {{ $systemSettings['currencySymbol'] }}{{ number_format((float) $addon->price, 2) }}
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                    ...
                    <td>{{ $systemSettings['currencySymbol'] }}{{ number_format($item?->orderItem['subtotal'], 2) }}</td>
                </tr>
            @endforeach
            </tbody>
            <tfoot>
