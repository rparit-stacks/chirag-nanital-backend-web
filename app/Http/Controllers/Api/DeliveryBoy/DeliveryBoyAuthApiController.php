...
class DeliveryBoyAuthApiController extends Controller
{
    ...
    protected string $role = 'delivery_boy';
    ...
    public function register(RegisterDeliveryBoyRequest $request): JsonResponse
    {
        try {
            ...
            // Auto-generate a unique referral code for the new delivery boy
            app(DeliveryBoyReferralService::class)->generateAndAssignCode($deliveryBoy);

            // If they entered a friends_code, create the pending referral record
            if (!empty($validated['friends_code'])) {
                app(DeliveryBoyReferralService::class)->handleRegistration($deliveryBoy, $validated['friends_code']);
            }
    ...
    /** Finalize successful login via trait (like phoneCallback) */
    protected function finalizeLogin(Request $request, ?User $user = null): JsonResponse
    {
        $user = $user ?? $request->user();
        if (!$user) {
            return ApiResponseType::sendJsonResponse(false, __('labels.invalid_credentials'));
        }
        $this->storeFcmToken($request, $user);
    ...
    public function getProfile(Request $request): JsonResponse
    {
        try {
            ...
            return ApiResponseType::sendJsonResponse(
                ...
                data: [
                    'user' => (new UserResource($user))->withWalletType(WalletTypeEnum::DELIVERY_BOY),
    ...
    public function updateStatus(Request $request): JsonResponse
    {
        try {
            ...
            if ($validated['status'] === ActiveInactiveStatusEnum::ACTIVE()) {
                ...
                $areCoordsValid = DeliveryZoneService::validateCoordinates(
                ...
                // Find zone by current coordinates and update delivery boy's zone
                $zoneInfo = DeliveryZoneService::getZonesAtPoint((float)$validated['latitude'], (float)$validated['longitude']);
                if (!$zoneInfo['exists'] || empty($zoneInfo['zone_id'])) {
                    return ApiResponseType::sendJsonResponse(
                        success: false,
            ...
            // Update status (and zone when going active)
            if ($validated['status'] === ActiveInactiveStatusEnum::ACTIVE()) {
                // Safe to reference $zoneInfo here because we validated in the ACTIVE branch above
