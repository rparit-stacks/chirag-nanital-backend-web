...
class DeliveryZoneApiController extends Controller
{
    ...
    public function checkDelivery(Request $request): JsonResponse
    {
        ...
        // If deliverable and a user is authenticated, upsert single mapping per user (user_id unique)
        if ($isDeliverable) {
            $user = Auth::guard('sanctum')->user();
            $zoneId = $zoneInfo['zone_id'] ?? null;

            if ($user && $zoneId) {
                $user->deliveryZones()->sync([$zoneId]);
            }
        }
        $response = [
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        $zones = DeliveryZone::where('name', 'like', '%' . $query . '%')
