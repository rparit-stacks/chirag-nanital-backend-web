...
class SellerWalletApiController extends Controller
{
    ...
    public function show(): JsonResponse
    {
        ...
        $result = $this->walletService->getWallet($seller->user_id ?? $seller->user->id, WalletTypeEnum::SELLER);
        if (!$result['success']) {
    ...
    public function transactions(Request $request): JsonResponse
    {
        ...
        $filters = $request->all();
        $result = $this->walletService->getTransactions($seller->user_id ?? $seller->user->id, $filters, WalletTypeEnum::SELLER);
        if (!$result['success']) {
            return ApiResponseType::sendJsonResponse(false, $result['message'], $result['data'] ?? []);
        }
    ...
    public function transaction(int $id): JsonResponse
    {
        ...
        $result = $this->walletService->getTransaction($seller->user_id ?? $seller->user->id, $id, WalletTypeEnum::SELLER);
        if (!$result['success']) {
            return ApiResponseType::sendJsonResponse(false, $result['message'], $result['data'] ?? []);
        }
