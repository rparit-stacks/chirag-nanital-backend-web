...
class DeliveryBoySettlementSettled extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): ?MailMessage
    {
        try {
            ...
            $mail = (new MailMessage)
                ->subject($isAdmin ? 'Delivery Boy Earning Settled' : 'Earning Settled')
                ->greeting('Hello ' . ($notifiable->name ?? ''));

            if ($isAdmin) {
                $mail->line('A delivery boy earning has been marked as settled.')
                    ->line('Delivery Boy: ' . $riderName)
                    ->line('Order ID: ' . ($a->order_id ?? '-'))
                    ->line('Amount: ' . $amount)
                    ->line('Paid At: ' . optional($a->paid_at)->toDateTimeString());
            } else {
                $mail->line('Your delivery earning has been settled to your wallet.')
                    ->line('Order ID: ' . ($a->order_id ?? '-'))
                    ->line('Amount: ' . $amount)
                    ->line('Paid At: ' . optional($a->paid_at)->toDateTimeString());
            }
    ...
    public function toFirebase($notifiable): array
    {
        ...
        return [
            'title' => $isAdmin ? 'Delivery Earning Settled' : 'Earning Settled',
            'body'  => $isAdmin
                ? ($riderName . "'s earning of " . $amount . ' settled for Order #' . ($a->order_id ?? '-') . '.')
                : ('Amount ' . $amount . ' settled for Order #' . ($a->order_id ?? '-') . '.'),
            'image' => null,
            'data'  => [
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            'title' => $isAdmin ? 'Delivery Earning Settled' : 'Earning Settled',
            'message' => $isAdmin
                ? ('Delivery earning of ' . $amount . ' has been settled to ' . $riderName . ' for Order #' . ($a->order_id ?? '-') . '.')
                : ('Your earning of ' . $amount . ' has been settled for Order #' . ($a->order_id ?? '-') . '.'),
            'type' => NotificationTypeEnum::SETTLEMENT_PROCESS(),
